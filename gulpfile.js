...
var dirs = pkg['flexui-configs'].directories;
...
gulp.task('pug', function() {
    return gulp.src('src/doc/*.pug')
        .pipe(pug())
        .pipe(gulp.dest(dirs.src + '/doc'))
        .pipe(reload({ stream:true }))
        .pipe(plugins.notify({ message: 'doc task completed' }));
});
...
gulp.task('watch', function () {
    ...
    // Watch .pug files
    gulp.watch(dirs.src + '/doc/*.pug', ['pug']);

    // Watch .html files
    gulp.watch(dirs.src + '/doc/*.html', ['run:html']);
    // gulp.watch(dirs.src + '/doc/*.html').on('change', reload);
...
gulp.task('build', function (done) {
    runSequence(
        ['clean', 'lint:js', 'styles', 'scripts', 'pug'],
        'copy', 'clean:src',
    done);
});
